<?php
    ...
    class bd{
        ...
        public function agregar_alummno($alummno){
        ...
        public function agregar_maestro($maestro){
           try{ $query=$this->mbd->prepare('INSERT INTO maestros(nombre,paterno,materno,no_empleado,telefono,carrera) 
                                        VALUES(:nombre,:paterno,:materno,:no_empleado,:telefono,:carrera)');
            $query->bindParam(':nombre',$maestro->nombre,PDO::PARAM_STR);
            $query->bindParam(':paterno',$maestro->paterno,PDO::PARAM_STR);
...
<?php
    ...
    if(isset($_POST['btn_save'])){
        $maestro = new maestro($_POST['nombre'],$_POST['paterno'],$_POST['materno'],
        $_POST['no_empleado'],$_POST['telefono'],$_POST['carrera']);
        $obj_bd = new bd("root","","localhost","practica_2");
        $obj_bd->agregar_maestro($maestro);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        ...
        <input type="text" required name="no_empleado" id="no_empleao">
